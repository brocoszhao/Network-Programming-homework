// blocksevr.cpp : Defines the entry point for the console application.
//
	...
	while(1){
		...
		for(i = 0;i < 64;i++){
			...
			if(FD_ISSET(sock,&readfds)){
				...
				printf("the second recv function to make the server blocked");
				retval = recv(sock,buf,128,0); //the second recv function to make the server blocked
				printf("%d",retval);
				if(retval == 0){
					closesocket(sock);
					printf("close a socket\n");
